...
| 2026-07 | Deploy Monolítico (Backend Go servindo o frontend React) em vez de Deploy Desacoplado | Evita a complexidade operacional de gerenciar múltiplos pipelines de deploy, domínios separados e configuração de CORS em produção. Como a API utilizará o free tier do Render (que hiberna após inatividade), o gargalo do *cold start* ocorrerá de qualquer forma. Manter o frontend e o backend no mesmo serviço simplifica o MVP e a manutenção para um desenvolvedor iniciante nessas stacks. O desacoplamento só será feito futuramente caso a necessidade de escala justifique. |
