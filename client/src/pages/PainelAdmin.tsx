...
import DestaqueListItem from '../components/DestaqueListItem'
...
export default function PainelAdmin() {
  ...
  // Antes cada chamador (cover/banner/char) empurrava o resultado direto pro
  // estado global via um parâmetro `tipo`. Agora a função só faz upload e
  // devolve a URL (ou null) — quem chama decide onde ela vai.
  ...
  return (
    <div className="min-h-screen bg-void text-text pb-20 relative z-10">
      ...
      <div className="max-w-[1000px] mx-auto p-4 sm:p-5 mt-4 relative z-10">
        <div className="mb-6">
        ...
        <div className="bg-panel border border-line rounded-2xl p-4 sm:p-6 mb-8 shadow-xl">
          ...
          {previewTitulo && (
            <div className="border-t border-dashed border-line pt-6 mt-4 space-y-6 animate-in fade-in duration-300">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 p-2 border border-line rounded-xl bg-panel-2 min-h-[44px] items-center">
                  {tags.map((tag) => (
                    ...
                    >
                      {tag}{' '}
                      <button type="button" onClick={() => removerTag(tag)} className="hover:text-white opacity-70 ml-1 cursor-pointer">
                        ×
                      </button>
                    </span>
                  ))}
              ...
              {/* Mobile: botão de publicar vem primeiro (ação principal),
                  "Ordem Home" fica como campo secundário abaixo. */}
            ...
            {destaques.length === 0 ? (
              ...
              destaques.map((anime, index) => (
                <DestaqueListItem
                  key={anime.id}
                  anime={anime}
                  gradClass={`card-g${(index % 5) + 1}`}
                  onEdit={editarDestaque}
