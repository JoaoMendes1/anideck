...
export default function RankingCard({ anime, rank, isSaved, isFavorite, isSaving, onToggleSave }: RankingCardProps) {
    ...
    return (
        <Link
            ...
            className={`relative overflow-hidden flex items-center gap-2.5 sm:gap-3 p-2.5 sm:p-3 rounded-xl transition-colors group ${
                isFavorite
                    ? 'foil-card border border-gold/50 shadow-[0_0_15px_rgba(255,197,66,0.15)]'
                    : 'bg-panel border border-line hover:border-holo-2'
            }`}
        >
            {/* COLUNA ESQUERDA: Capa e Badge */}
            ...
            <div className="relative z-30 flex flex-col items-center justify-between shrink-0 w-[42px] sm:w-[48px] h-full gap-2">
                ...
                <button
                    ...
                    className={`flex w-[26px] h-[26px] sm:w-7 sm:h-7 rounded-full border-[1.5px] items-center justify-center font-bold text-base transition-colors select-none ${
                        isSaved
